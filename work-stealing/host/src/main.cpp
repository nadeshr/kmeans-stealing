...
int main() {
    ...
    uint stolen_nodes[L][P];
    uint stries[L][P];
    uint centre_nodes[L][P];
    uint allocs_set[L][P];
    ...
    posix_memalign ((void**)(&stolen_nodes), 64, sizeof(stolen_nodes));
    posix_memalign ((void**)(&stolen_nodes), 64, sizeof(centre_nodes));
    ...
    cl_int max_tries = 0 ;
    cl_int delay_cycles = 0; 
    cl_int backoff_cycles = 0;

    uint delay_accumulator[P];
    cl_mem acc_buffer; 
    acc_buffer = clCreateBuffer(context, CL_MEM_READ_WRITE, 
            sizeof(delay_accumulator), NULL , &err);
    if(err < 0) {
        perror("Couldn't create a buffer");
        return -1;   
    }
    ...
    cl_mem heap_buffer; 
    heap_buffer = clCreateBuffer(context, CL_MEM_READ_WRITE, 
    ...
    cl_mem list_buffer; 
    list_buffer = clCreateBuffer(context, CL_MEM_READ_WRITE, 
    ...
    cl_mem stolen_nodes_buffer; 
    stolen_nodes_buffer = clCreateBuffer(context, CL_MEM_WRITE_ONLY, 
            sizeof(stolen_nodes), NULL , &err);
    if(err < 0) {
        perror("Couldn't create a buffer");
        return -1;   
    }

    cl_mem stries_buffer; 
    stries_buffer = clCreateBuffer(context, CL_MEM_WRITE_ONLY, 
            sizeof(stries), NULL , &err);
    if(err < 0) {
        perror("Couldn't create a buffer");
        return -1;   
    }

    cl_mem centre_nodes_buffer; 
    centre_nodes_buffer = clCreateBuffer(context, CL_MEM_WRITE_ONLY, 
            sizeof(centre_nodes), NULL , &err);
    if(err < 0) {
        perror("Couldn't create a buffer");
        return -1;   
    }

    cl_mem allocs_set_buffer; 
    allocs_set_buffer = clCreateBuffer(context, CL_MEM_WRITE_ONLY, 
            sizeof(allocs_set), NULL , &err);
    if(err < 0) {
        perror("Couldn't create a buffer");
        return -1;   
    }

    char stealvar_str[256];
#ifdef NEAREST_NEIGHBOUR
    sprintf(stealvar_str,"neighbour");
#elif defined CYCLIC 
    sprintf(stealvar_str,"cyclic");
#else
    sprintf(stealvar_str,"crossbar");
#endif

    char implvar_str[256];
#ifdef SIMD
    sprintf(implvar_str,"lockstepWS");
#else 
    sprintf(implvar_str,"completeWS");
#endif
    ...
    for (uint runs=0; runs<2; runs++) {
        ...
        err = clSetKernelArg(kernel, arg++, sizeof(cl_mem), &stolen_nodes_buffer);
        if (err != CL_SUCCESS) {
            return -1;
        };

        err = clSetKernelArg(kernel, arg++, sizeof(cl_mem), &centre_nodes_buffer);
        if (err != CL_SUCCESS) {
            return -1;
        };
        ...
        err = clEnqueueReadBuffer(queue, stolen_nodes_buffer, CL_TRUE, 0,sizeof(stolen_nodes), stolen_nodes, 0, 0, 0);
        if (err != CL_SUCCESS) {
            return -1;
        }


        err = clEnqueueReadBuffer(queue, centre_nodes_buffer, CL_TRUE, 0,sizeof(centre_nodes), centre_nodes, 0, 0, 0);
        if (err != CL_SUCCESS) {
            return -1;
        }
        ...
        if(runs==0){        
            printf("\nBalanced tree stats for work-stealing k-means\n");
        }else{
            printf("\nUnbalanced tree stats for work-stealing k-means\n");
        }
        ...
        for(uint l=0; l < L; l++){
            ...
            uint total = 0;
            for(uint p=0; p<P; p++){
                printf("%d\t",centre_nodes[l][p] );
                total+=centre_nodes[l][p];
            }
            printf("\t = %d\n",total);
        } 
...
bool init() {
    ...
    // Create the program.
    std::string binary_file = getBoardBinaryFile("filtering_algorithm_work_stealing_top", device);
    printf("Using AOCX: %s\n", binary_file.c_str());
    ...
    const char *kernel_name = "filtering_algorithm_work_stealing_top";  // Kernel name, as defined in the CL file
